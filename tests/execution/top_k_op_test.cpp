...
TEST(TopKOperatorTest, SortsAscAcrossBatches) {
    TopKOperator op(MakeValueInput({{5, 1, 8}, {3, 2}, {7}}),
...
TEST(TopKOperatorTest, SortsDescAcrossBatches) {
    TopKOperator op(MakeValueInput({{5, 1, 8}, {3, 2}, {7}}),
                    {SortKey{"value", SortOrder::DESC}}, 3);

    std::optional<ExecBatch> batch = op.Next();
    ASSERT_TRUE(batch.has_value());
    EXPECT_EQ(std::get<std::vector<int64_t>>(batch->columns[0]),
              std::vector<int64_t>({8, 7, 5}));
    EXPECT_EQ(batch->row_count, 3u);
}

TEST(TopKOperatorTest, SortsByTwoKeys) {
    auto schema =
...
TEST(TopKOperatorTest, AllowsLimitLargerThanInput) {
    TopKOperator op(MakeValueInput({{4, 1}, {3}}),
                    {SortKey{"value", SortOrder::ASC}}, 10);

    std::optional<ExecBatch> batch = op.Next();
    ASSERT_TRUE(batch.has_value());
    EXPECT_EQ(std::get<std::vector<int64_t>>(batch->columns[0]),
              std::vector<int64_t>({1, 3, 4}));
    EXPECT_EQ(batch->row_count, 3u);
}
